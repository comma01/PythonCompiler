    ...
    for w_it in words:
        ...
        if digit(w_it[0]) == TRUE or ((w_it[0] == '+' or w_it[0] == '-') and digit(w_it[1]) == TRUE) or w_it[0] == '.':
            if decimal(w_it) == TRUE:
                ...
                error.append([w_it, 'is not a float in line', str(row)])
        # 判断id和keyword
        elif identifier_letter(w_it[0]) == TRUE or underline(w_it[0]) == TRUE:
            if identifier(w_it) == TRUE:
                if keywords(w_it) == TRUE:
                    result.append(['KEYWORD', w_it]) 
                else:
                    result.append(['IDENTIFIER', w_it])
            else:
                error.append([w_it, 'is not an identifier in line', str(row)])
        # 判断特殊符号
        else:
            if specialSymbol(w_it) == TRUE:
                result.append(['SPECIALSYMBOL', w_it])
            else:
                error.append([w_it, 'is not an special symbol in line', str(row)])
...
if __name__ == '__main__':
    #print(decimal('50'))
    result, error = solve('end Thread6;', 1)